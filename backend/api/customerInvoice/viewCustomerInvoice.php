...
class PDF extends FPDF
{
	...
	{
		$thisDate = implode('/', array_reverse(explode('-', substr($thisCustomerInvoice['dateDailyPayment'], 0, 10))));
		//alors pour le numéro de facture la comptable veut impérativement date AAAAMMJJN-NC  soit 202105061-1, 202105062-2, .... 20210507-3  
		//soit le N ce remet à zéro tous les jours et le NC ne se remet jamais à zéro.
		$invoiceNumber = str_replace('-', '', substr($thisCustomerInvoice['dateDailyPayment'], 0, 10)) . N .'-'. ;
	    ...
	    $this->Ln(0);
	    $this->Cell(70);
	    $this->Cell(30,10,'N° : '. $invoiceNumber,1,0,'C');
	    $this->Ln(0);
	    $this->Cell(70);
	    $this->Cell(30,10,'Date : '. $thisDate,1,0,'C');
	    $this->Ln(20);
	}
	...
	{
		$this->Cell(80);
		$this->Cell(40, 10,"Total hors taxe : ",1,0,'L');
		$this->Cell(0,0,$totalHtAmount,1,0,'R');
		$this->Cell(80);
